...
struct Args {
    ...
    #[arg(
        short,
        long,
        help = "Do not automatically correct interrupt address to account for ARM-thumb mode"
    )]
    do_not_correct: bool,
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let offset = (section.sh_offset + (symbol_address - section.sh_addr)) as usize;

    let mut entry = args.n_th_entry.unwrap_or(0);
    if args.n_th_entry.is_none() {
        // we start at 16 because that's where the interrupts start in the vector table
        let found_entry = (16usize..section.sh_size as usize).find(|i| {
    ...
    if !args.do_not_correct {
        interrupt_address = interrupt_address_from_sym.unwrap_or(args.interrupt_address);

        if args.interrupt_address != interrupt_address {
            println!(
                "Automatically corrected interrupt address to account for ARM-thumb mode. Pass -d to disable correction."
            );
        }
    }
    ...
    println!("Patched bytes at file offset 0x{offset:x}");
